...
function AddBranchModal({ isOpen, onClose, onSubmit, isRtl }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={() => { if (!loading) { reset(); onClose(); } }}>
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 border border-[#D4AF37]/20 bg-[#161616] shadow-2xl overflow-hidden animate-[fadeIn_0.2s_ease]"
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="px-8 py-6 border-b border-[#D4AF37]/20 flex items-center justify-between bg-black/20">
          <div>
            <h3 className="text-lg font-black text-zinc-100 uppercase tracking-wider">
          ...
          <button onClick={() => { if (!loading) { reset(); onClose(); } }}
            className="w-10 h-10 flex items-center justify-center text-zinc-400 hover:text-zinc-100 hover:bg-white/5 transition-colors text-xl">✕</button>
        </div>
        ...
        <div className="p-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              ...
              className="w-full bg-black/40 border border-[#D4AF37]/20 px-5 py-4 text-zinc-100 text-sm font-bold outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/50 transition-all placeholder:text-zinc-600" />
          </div>

          <div>
            <label className="block text-[9px] font-black text-zinc-400 uppercase tracking-[3px] mb-2">
        ...
        {/* Footer */}
        <div className="px-8 py-6 border-t border-[#D4AF37]/20 bg-black/20 flex items-center justify-between gap-4">
          <button onClick={() => { reset(); onClose(); }} disabled={loading}
            className="px-6 py-3 text-zinc-400 font-black text-[10px] uppercase tracking-widest hover:text-zinc-100 transition-colors">
            {isRtl ? 'إلغاء' : 'Cancel'}
          </button>
...
export default function BranchManagement({ language }) {
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 min-h-full bg-[#0a0a0c]" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-[#161616] border border-[#D4AF37]/20 p-5 flex items-center gap-4 hover:border-[#D4AF37]/40 transition-colors shadow-lg">
            ...
            <div>
              <p className="text-2xl font-black text-zinc-100 tracking-tight">{loading ? '—' : stat.value}</p>
      ...
      {/* Branches Table */}
      <div className="bg-[#161616] border border-[#D4AF37]/20 shadow-lg overflow-hidden">
        {/* Table Header */}
        <div className="px-6 py-4 border-b border-[#D4AF37]/20 flex items-center justify-between bg-black/20">
          ...
          <button onClick={loadBranches} disabled={loading}
            className="text-[10px] font-black text-zinc-400 uppercase tracking-widest hover:text-[#D4AF37] transition-colors flex items-center gap-2">
        ...
        {!loading && branches.length > 0 && (
          <div className="divide-y divide-white/5">
            {/* Column Headers */}
            <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-black/40 border-b border-[#D4AF37]/10">
              <div className="col-span-1 text-[8px] font-black text-zinc-500 uppercase tracking-widest">#</div>
              <div className="col-span-3 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{isRtl ? 'الفرع' : 'Branch'}</div>
              <div className="col-span-3 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{isRtl ? 'العنوان' : 'Address'}</div>
              <div className="col-span-2 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{isRtl ? 'الهاتف' : 'Phone'}</div>
              <div className="col-span-1 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{isRtl ? 'الحالة' : 'Status'}</div>
              <div className="col-span-2 text-[8px] font-black text-zinc-500 uppercase tracking-widest text-end">{isRtl ? 'إجراءات' : 'Actions'}</div>
            </div>

            {branches.map((branch, idx) => (
              <div key={branch.id}
                className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-black/20 transition-colors group ${!branch.is_active ? 'opacity-40' : ''}`}>
                {/* Index */}
                ...
                <div className="col-span-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-black text-zinc-100 truncate">{branch.name}</p>
                    <p className="text-[9px] text-zinc-400 font-bold truncate">ID: {branch.id?.slice(0, 8)}</p>
                  </div>
                </div>

                {/* Address */}
                <div className="col-span-3">
                  <p className="text-xs text-zinc-400 font-bold truncate">{branch.address || '—'}</p>
                </div>

                {/* Phone */}
                <div className="col-span-2">
                  <p className="text-xs text-zinc-400 font-bold">{branch.phone || '—'}</p>
                </div>
