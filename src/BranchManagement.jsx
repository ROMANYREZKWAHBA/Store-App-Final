...
function AddBranchModal({ isOpen, onClose, onSubmit, isRtl }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={() => { if (!loading) { reset(); onClose(); } }}>
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 border border-[#333] bg-[#0c0c0e] overflow-hidden animate-[fadeIn_0.2s_ease]"
        ...
        <div className="px-8 py-6 border-b border-[#222] flex items-center justify-between">
          ...
          <button onClick={() => { if (!loading) { reset(); onClose(); } }}
            className="w-10 h-10 flex items-center justify-center text-[#666] hover:text-white hover:bg-[#1a1a1a] transition-colors text-xl">✕</button>
        </div>
        ...
        <div className="p-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              ...
              className="w-full bg-[#111] border border-[#333] px-5 py-4 text-white text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors placeholder:text-[#444]" />
          </div>

          <div>
            <label className="block text-[9px] font-black text-[#888] uppercase tracking-[3px] mb-2">
        ...
        <div className="px-8 py-6 border-t border-[#222] bg-[#080809] flex items-center justify-between gap-4">
          <button onClick={() => { reset(); onClose(); }} disabled={loading}
            className="px-6 py-3 text-[#666] font-black text-[10px] uppercase tracking-widest hover:text-white transition-colors">
            {isRtl ? 'إلغاء' : 'Cancel'}
          </button>
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 min-h-full" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: isRtl ? 'إجمالي الفروع' : 'Total Branches', value: branches.length, color: '#0066FF', icon: '🏢' },
          { label: isRtl ? 'فروع نشطة' : 'Active', value: activeCount, color: '#10b981', icon: '✅' },
          { label: isRtl ? 'فروع معطلة' : 'Inactive', value: branches.length - activeCount, color: '#ef4444', icon: '⏸️' },
          { label: isRtl ? 'الخزائن المُزوّدة' : 'Safes Provisioned', value: branches.length, color: '#D4AF37', icon: '🏦' },
        ].map((stat, i) => (
      ...
      <div className="bg-[#0c0c0e] border border-[#222] overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-[#222] flex items-center justify-between bg-[#111]">
          ...
          <button onClick={loadBranches} disabled={loading}
            className="text-[10px] font-black text-[#666] uppercase tracking-widest hover:text-[#D4AF37] transition-colors flex items-center gap-2">
        ...
        {!loading && branches.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-[#666] text-sm font-black uppercase tracking-widest">
        ...
        {!loading && branches.length > 0 && (
          <div className="divide-y divide-[#1a1a1a]">
            {/* Column Headers */}
            <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-[#0a0a0a]">
              <div className="col-span-1 text-[8px] font-black text-[#555] uppercase tracking-widest">#</div>
              <div className="col-span-3 text-[8px] font-black text-[#555] uppercase tracking-widest">{isRtl ? 'الفرع' : 'Branch'}</div>
              <div className="col-span-3 text-[8px] font-black text-[#555] uppercase tracking-widest">{isRtl ? 'العنوان' : 'Address'}</div>
              <div className="col-span-2 text-[8px] font-black text-[#555] uppercase tracking-widest">{isRtl ? 'الهاتف' : 'Phone'}</div>
              <div className="col-span-1 text-[8px] font-black text-[#555] uppercase tracking-widest">{isRtl ? 'الحالة' : 'Status'}</div>
              <div className="col-span-2 text-[8px] font-black text-[#555] uppercase tracking-widest text-end">{isRtl ? 'إجراءات' : 'Actions'}</div>
            </div>

            {branches.map((branch, idx) => (
              <div key={branch.id}
                className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-[#111] transition-colors group ${!branch.is_active ? 'opacity-40' : ''}`}>
                {/* Index */}
                <div className="col-span-1">
                  <span className="text-xs font-black text-[#333]">{String(idx + 1).padStart(2, '0')}</span>
                </div>

                {/* Name */}
                <div className="col-span-3 flex items-center gap-3">
                  <div className="w-9 h-9 flex items-center justify-center shrink-0"
                    style={{ background: branch.is_active ? '#D4AF37' : '#333', color: branch.is_active ? '#000' : '#666', fontWeight: 900, fontSize: 14 }}>
                    {branch.name?.[0]?.toUpperCase() || 'B'}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-black text-white truncate">{branch.name}</p>
                    <p className="text-[9px] text-[#555] font-bold truncate">ID: {branch.id?.slice(0, 8)}</p>
                  </div>
                </div>

                {/* Address */}
                <div className="col-span-3">
                  <p className="text-xs text-[#888] font-bold truncate">{branch.address || '—'}</p>
                </div>

                {/* Phone */}
                <div className="col-span-2">
                  <p className="text-xs text-[#888] font-bold">{branch.phone || '—'}</p>
                </div>
