...
function NewTransferModal({ isOpen, onClose, onSubmit, items, currentBranchId, isRtl, currentUser }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={() => !loading && onClose()}>
      ...
      <div className="relative w-full max-w-lg mx-4 border border-zinc-200 dark:border-[#D4AF37]/20 bg-white dark:bg-[#151518] shadow-2xl overflow-hidden animate-[fadeIn_0.2s_ease]" onClick={e => e.stopPropagation()}>
        <div className="px-8 py-6 border-b border-zinc-200 dark:border-[#D4AF37]/20 bg-slate-50 dark:bg-[#0a0a0c] flex items-center justify-between">
          <div>
          ...
          <button onClick={() => !loading && onClose()} className="w-10 h-10 flex items-center justify-center text-slate-500 dark:text-[#666] hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors text-xl">✕</button>
        </div>
        ...
        <div className="p-8 space-y-5">
          ...
          <div>
            ...
            <select value={toBranchId} onChange={e => setToBranchId(e.target.value)}
              className="w-full bg-slate-50 dark:bg-black/40 border border-zinc-200 dark:border-[#D4AF37]/20 px-5 py-4 text-slate-900 dark:text-zinc-100 text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors"
              style={{ borderColor: toBranchId ? '#D4AF37' : '' }}>
              <option value="" className="bg-white dark:bg-[#151518] text-slate-900 dark:text-zinc-100">{isRtl ? '— اختر الفرع —' : '— Select Branch —'}</option>
              {branches.map(b => <option key={b.id} value={b.id} className="bg-white dark:bg-[#151518] text-slate-900 dark:text-zinc-100">{b.name}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            <select value={itemId} onChange={e => setItemId(e.target.value)}
              className="w-full bg-slate-50 dark:bg-black/40 border border-zinc-200 dark:border-[#D4AF37]/20 px-5 py-4 text-slate-900 dark:text-zinc-100 text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors">
              <option value="" className="bg-white dark:bg-[#151518] text-slate-900 dark:text-zinc-100">{isRtl ? '— اختر الصنف —' : '— Select Item —'}</option>
              {items.filter(i => (i.stock || 0) > 0).map(i => (
                <option key={i.id} value={i.id} className="bg-white dark:bg-[#151518] text-slate-900 dark:text-zinc-100">{i.name?.[isRtl ? 'ar' : 'en'] || i.name?.en} ({isRtl ? 'متوفر' : 'Stock'}: {i.stock})</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input type="number" min="1" max={selectedItem?.stock || 9999} value={quantity} onChange={e => setQuantity(e.target.value)}
              placeholder="0" className="w-full bg-slate-50 dark:bg-black/40 border border-zinc-200 dark:border-[#D4AF37]/20 px-5 py-4 text-slate-900 dark:text-zinc-100 text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors placeholder:text-slate-400 dark:placeholder:text-[#444]" />
          </div>

          <div>
            <label className="block text-[9px] font-black text-[#888] uppercase tracking-[3px] mb-2">{isRtl ? 'ملاحظات' : 'Notes'}</label>
            <input type="text" value={notes} onChange={e => setNotes(e.target.value)}
              placeholder={isRtl ? 'اختياري...' : 'Optional...'} className="w-full bg-slate-50 dark:bg-black/40 border border-zinc-200 dark:border-[#D4AF37]/20 px-5 py-4 text-slate-900 dark:text-zinc-100 text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors placeholder:text-slate-400 dark:placeholder:text-[#444]" />
          </div>
        </div>
...
export default function StockTransfersScreen({ currentUser, branchId, items, language, pushNotification }) {
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 min-h-full bg-white dark:bg-[#0a0a0c] text-slate-900 dark:text-zinc-100 transition-colors duration-200" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white dark:bg-[#151518] border border-zinc-200 dark:border-[#D4AF37]/20 p-5 flex items-center gap-4 hover:border-[#D4AF37]/40 transition-colors shadow-lg">
            <div className={`w-12 h-12 rounded-none flex items-center justify-center text-2xl bg-slate-50 dark:bg-black/30 border border-zinc-200 dark:border-white/5 ${s.color}`}>
              {s.icon}
            </div>
            <div>
      ...
      {/* Table */}
      <div className="bg-white dark:bg-[#151518] border border-zinc-200 dark:border-[#D4AF37]/20 overflow-hidden shadow-lg transition-colors duration-200">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-[#D4AF37]/20 flex items-center justify-between bg-slate-50 dark:bg-[#0a0a0c]">
          ...
          <button onClick={loadTransfers} disabled={loading} className="text-[10px] font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest hover:text-[#D4AF37] transition-colors flex items-center gap-2">
            <span className={loading ? 'animate-spin' : ''}>🔄</span> {isRtl ? 'تحديث' : 'Refresh'}
          </button>
        </div>

        {loading && (
          <div className="flex items-center justify-center py-20 gap-4 bg-transparent">
            <div className="w-6 h-6 border-2 border-zinc-200 dark:border-[#D4AF37]/20 border-t-[#D4AF37] animate-spin" style={{ borderRadius: '50%' }} />
            <p className="text-slate-500 dark:text-zinc-400 text-xs font-black uppercase tracking-widest">{isRtl ? 'جاري التحميل...' : 'Loading...'}</p>
          </div>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4 bg-transparent">
            ...
            <p className="text-slate-500 dark:text-zinc-400 text-sm font-black uppercase tracking-widest">{isRtl ? 'لا توجد تحويلات' : 'No transfers found'}</p>
          </div>
        )}

        {!loading && filtered.length > 0 && (
          <div className="divide-y divide-zinc-200 dark:divide-white/5 bg-transparent">
            {filtered.map((t) => {
              ...
              return (
                <div key={t.id} className="px-6 py-5 hover:bg-slate-50 dark:hover:bg-black/20 transition-colors group">
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className="text-sm font-black text-slate-900 dark:text-zinc-100 truncate">{itemName || t.item_id}</p>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 dark:text-zinc-400 font-bold flex-wrap">
                          <span>{isRtl ? 'من:' : 'From:'} <span className="text-slate-805 dark:text-[#888]">{t.from_branch?.name || '—'}</span></span>
                          <span className="text-slate-300 dark:text-[#333]">→</span>
                          <span>{isRtl ? 'إلى:' : 'To:'} <span className="text-slate-805 dark:text-[#888]">{t.to_branch?.name || '—'}</span></span>
                        </div>
                        {t.notes && <p className="text-[10px] text-slate-500 dark:text-[#555] mt-1 italic">{t.notes}</p>}
                        <p className="text-[9px] text-slate-400 dark:text-[#444] mt-1">{new Date(t.created_at).toLocaleString()}</p>
                      </div>
                    </div>
