    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center" onClick={() => !loading && onClose()}>
      ...
      <div className="relative w-full max-w-lg mx-4 border border-[#333] bg-[#0c0c0e] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-8 py-6 border-b border-[#222] flex items-center justify-between">
          <div>
            <h3 className="text-lg font-black text-white uppercase tracking-wider">{isRtl ? '📦 طلب تحويل مخزون' : '📦 New Stock Transfer'}</h3>
            <p className="text-[10px] text-[#666] font-bold uppercase tracking-widest mt-1">{isRtl ? 'سيتم إرسال الطلب للفرع المستلم للموافقة' : 'Requires receiver branch approval'}</p>
          </div>
          <button onClick={() => !loading && onClose()} className="w-10 h-10 flex items-center justify-center text-[#666] hover:text-white hover:bg-[#1a1a1a] transition-colors text-xl">✕</button>
        </div>
        ...
        <div className="p-8 space-y-5">
          ...
          <div>
            ...
            <select value={toBranchId} onChange={e => setToBranchId(e.target.value)}
              className="w-full bg-[#111] border border-[#333] px-5 py-4 text-white text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors"
          ...
          <div>
            ...
            <input type="number" min="1" max={selectedItem?.stock || 9999} value={quantity} onChange={e => setQuantity(e.target.value)}
              placeholder="0" className="w-full bg-[#111] border border-[#333] px-5 py-4 text-white text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors placeholder:text-[#444]" />
          </div>

          <div>
            <label className="block text-[9px] font-black text-[#888] uppercase tracking-[3px] mb-2">{isRtl ? 'ملاحظات' : 'Notes'}</label>
            <input type="text" value={notes} onChange={e => setNotes(e.target.value)}
              placeholder={isRtl ? 'اختياري...' : 'Optional...'} className="w-full bg-[#111] border border-[#333] px-5 py-4 text-white text-sm font-bold outline-none focus:border-[#D4AF37] transition-colors placeholder:text-[#444]" />
          </div>
        </div>

        <div className="px-8 py-6 border-t border-[#222] bg-[#080809] flex items-center justify-between gap-4">
          <button onClick={onClose} disabled={loading} className="px-6 py-3 text-[#666] font-black text-[10px] uppercase tracking-widest hover:text-white transition-colors">{isRtl ? 'إلغاء' : 'Cancel'}</button>
    ...
    <div className="p-6 md:p-10 space-y-8 min-h-full" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-black text-white uppercase tracking-wider">{isRtl ? 'تحويلات المخزون' : 'Stock Transfers'}</h1>
          </div>
          <p className="text-xs text-[#666] font-bold uppercase tracking-widest">{isRtl ? 'نظام التحويل ثنائي الخطوات — إرسال واعتماد' : 'Two-Step Verification — Send & Approve'}</p>
        </div>
      ...
      <div className="flex gap-3">
        {[
          ...
          <button key={key} onClick={() => setFilter(key)}
            className={`px-6 py-3 font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 ${filter === key ? 'bg-[#1a1a1a] text-white border border-[#D4AF37]' : 'bg-[#0c0c0e] text-[#666] border border-[#222] hover:border-[#444]'}`}>
            {label}
            {badge > 0 && <span className="bg-emerald-500 text-white text-[8px] font-black px-2 py-0.5" style={{ borderRadius: '50px' }}>{badge}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="bg-[#0c0c0e] border border-[#222] overflow-hidden">
        <div className="px-6 py-4 border-b border-[#222] flex items-center justify-between bg-[#111]">
          ...
          <button onClick={loadTransfers} disabled={loading} className="text-[10px] font-black text-[#666] uppercase tracking-widest hover:text-[#D4AF37] transition-colors flex items-center gap-2">
        ...
        {loading && (
          <div className="flex items-center justify-center py-20 gap-4">
            ...
            <p className="text-[#666] text-xs font-black uppercase tracking-widest">{isRtl ? 'جاري التحميل...' : 'Loading...'}</p>
          </div>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-[#666] text-sm font-black uppercase tracking-widest">{isRtl ? 'لا توجد تحويلات' : 'No transfers found'}</p>
          </div>
        )}
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className="text-sm font-black text-white truncate">{itemName || t.item_id}</p>
                      ...
                      {isIncoming && isPending && (
                        <>
                          <button
                            ...
                            className="px-4 py-2.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-black text-[9px] uppercase tracking-widest hover:bg-emerald-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                          >
                            {actionLoading === t.id ? (
                          ...
                          <button
                            ...
                            disabled={actionLoading === t.id}
                            className="px-4 py-2.5 bg-rose-500/10 border border-rose-500/30 text-rose-400 font-black text-[9px] uppercase tracking-widest hover:bg-rose-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                          >
                            {isRtl ? 'رفض' : 'Reject'}
                          </button>
                        </>
